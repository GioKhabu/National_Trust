...
<div class="container news">
    ...
    <div class="row">
		...
		if($page>0){
			$page=(int)$page;
			$ptk=mysqli_query($baza,'select * from CraftGallery where  ID='.$page);
...
<div id="jssor_1" style="position: relative; margin: 16px auto; top: 0px; left: 0px; width: 800px; height: 600px;   overflow: hidden; visibility: hidden;"><!-- Loading Screen -->
...
<!-- #endregion Jssor Slider End -->
			...
			$ptkc=mysqli_query($baza,'select COUNT(*) as cc from CraftGallery ');
			$row=mysqli_fetch_array($ptkc,1);
			$Count=$row['cc'];
			// Paging($Count, $st, $RowsPerPage, $url='')
			$RowsPerPage=12;
			if(isset($_GET['st'])) $st=(int)$_GET['st']; else $st=0;
			$ptk=mysqli_query($baza,'select * from CraftGallery order by DTime desc limit '.$st.','.$RowsPerPage);
