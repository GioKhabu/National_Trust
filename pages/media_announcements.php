...
<div class="container news">
    ...
    <div class="row">
		...
		if($page>0){
			$page=(int)$page;
			$ptk=mysqli_query($baza,'select * from MediaAnnouncements where  ID='.$page);
...
<!-- #endregion Jssor Slider End -->
        ...
        <?
			...
			$ptkc=mysqli_query($baza,'select COUNT(*) as cc from MediaAnnouncements where 1=1  '.$tag);
			$row=mysqli_fetch_array($ptkc,1);
			$Count=$row['cc'];
			// Paging($Count, $st, $RowsPerPage, $url='')
			$RowsPerPage=12;
			if(isset($_GET['st'])) $st=(int)$_GET['st']; else $st=0;
			$ptk=mysqli_query($baza,$sql='select * from MediaAnnouncements where 1=1  '.$tag.'  order by DTime desc limit '.$st.','.$RowsPerPage);
        ...
        <div class="col-md-3 col-sm-4">
            <div class="Tags">
                ...
                <?
                if(isset($_GET['tag'])) $tag=(int)$_GET['tag']; else $tag=0;
                $ptk=mysqli_query($baza,'select * from Tags where TableName="MediaAnnouncements"');
                while($row=mysqli_fetch_array($ptk,1)){
